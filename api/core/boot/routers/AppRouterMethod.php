...
function AppRouterMethod ($version, $method, $args=[]) {
  ...
  // check version
  $versionsData = include __DIR__ . '/../../methods/versions.php';
  ...
  $methodsData = include __DIR__ . '/../../methods/' . $version . '/methods.php';
  ...
  $methodPath = __DIR__ . '/../../methods/' . $version . '/' . $methodPath . '.php';
  ...
  if (count($methodInfo['objects']) > 0) {
    try {
      foreach($methodInfo['objects'] as $object) {
        // objects path
        $objectsPath = __DIR__ . '/../../objects/' . $object . '.php';
